...
namespace SDRSharp.RTLTCP
{
    internal static class ClassFunctionsVirtualListView
    {
        ...
        /// <param name="nb"></param>
        /// <param name="lv"></param>
        internal static void AddElemToCache(ListViewItem[] cacheLv, bool firstToTop, int nb, ListViewItem lv)
        {
            if (firstToTop)
            {
                cacheLv[nb] = lv;
        ...
        internal static void CompleteList(ListViewItem[] cacheLv, int maxColCurrent)
        {
        ...
        internal static void AddNewLine(Dictionary<string, string> listData, Dictionary<string, int> cacheListColumns, ListViewItem lv)
        {
            foreach (KeyValuePair<string, string> _data in listData)
            {
                if (cacheListColumns.TryGetValue(_data.Key, out int indexColonne))          //get index column for _data.key found _data.Key
                {
                    //add subitems before item at indexColonne
                    for (int i = lv.SubItems.Count; i < indexColonne; i++)
                    {
                        _ = lv.SubItems.Add("");
                    }
        ...
        /// <param name="lv"></param>
        internal static void RefreshLine(Dictionary<string, string> listData, Dictionary<string, int> cacheListColumns, ListViewItem lv)
        {
            for (int i = lv.SubItems.Count; i < cacheListColumns.Count; i++)
            {
                _ = lv.SubItems.Add("");
            }
            ...
            foreach (KeyValuePair<string, string> _data in listData)
            {
                if (cacheListColumns.TryGetValue(_data.Key, out int indexColonne))
                {
                    lv.SubItems[indexColonne - 1].Text = _data.Value;
                }
            }
        ...
        internal static void AddColumn(Dictionary<string, string> listData, Dictionary<string, int> cacheListColumns, ListView lv, int maxColumn)
        {
            foreach (KeyValuePair<string, string> _data in listData)
            {
                bool ret = AddOneColumn(_data.Key, cacheListColumns, lv, maxColumn);
                if (ret)
                {
                    return;
                }
        ...
        internal static bool AddOneColumn(string _data, Dictionary<string, int> cacheListColumns, ListView lv, int maxColumn)
        {
        ...
        internal static bool AddOneColumnDevices(string _data, Dictionary<string, int> cacheListColumns, ListView lv, int maxColumn)
        {
            if (!cacheListColumns.ContainsKey(_data)) //new col
            {
                if (cacheListColumns.Count >= maxColumn)
        ...
        internal static void ResizeAllColumns(ListView lv)
        {
            ...
            for (int col = 0; col < cc.Count; col++)
            {
                if (cc[col].Text != "")
                {
        ...
        internal static bool SerializeText(string fileName, Dictionary<string, int> cacheListColumns, ListViewItem[] cacheLv, bool formatNumber, int nbMessage, bool sensDirect)
        {
            ...
            {
                _ = MessageBox.Show(e.Message, "Error export item fct(serializeText).File:" + fileName.ToString(), MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }

        }
        internal static string ProcessLine(ListViewItem it, bool formatNumber, NumberFormatInfo nfi, int nbColumn)
        {
            string line = string.Empty;
            int nColumn = 0;
        ...
        internal static string ProcessLineTxt(Dictionary<string, string> listData, bool formatNumber, NumberFormatInfo nfi, int nbColumn)
        {
            string line = string.Empty;
            int nColumn = 0;
            foreach (KeyValuePair<string, string> _data in listData)
